...
public interface CommentsDao {
	...
	/**
	 * CommentsVO 객체에 담겨진 자료를 DB에 insert하는 메서드
	...
	/**
	 * DB의 myComments테이블의 전체 레코드를 가져와서 List에 담아 반환하는 메서드
	 * @param smc SqlMapClient 객체
	 * @return 회원정보를 담고있는 List객체
	 * @throws SQLException JDBC관련 예외객체
	 */
	
	public List<CommentsVO> getAllCommentsList(SqlMapClient smc)
	 					throws SQLException;
	 ...
	 */
	public int updateComments(SqlMapClient smc, CommentsVO cv)
						throws SQLException;
	/**
	 ...
	 */
	public int deleteComments(SqlMapClient smc, String CommentsNo)
						throws SQLException;
	 ...
	 * @throws SQLException JDBC관련 예외 객체
	 */
	public List<CommentsVO> getSearchComments(SqlMapClient smc, CommentsVO cv) 
						throws SQLException;
	
	/**
	 * 주어진 회원 ID에 해당하는 회원정보를 조회하는 메서드
	 * @param smc SqlMapClient 객체
	 * @param CommentsNo 검색할 회원 ID
